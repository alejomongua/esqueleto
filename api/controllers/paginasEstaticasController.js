      ...
      usuarios.findByEmail(req.body.email,function(err, doc){ // Verifique que el correo exista
        if (err){
          ...
          if (doc){
            ...
            var newFechaToken = new Date(Date.now() + 8.64e7); // mañana a esta hora
            usuarios.update(doc.id, {token: newToken, fecha_token: newFechaToken}, null, function(err,usuario){
              if (err){              
                console.log(err);
                res.send(302, {
                  url: '/',
                ...
                // enviar correo
                var url = host + '/recuperar_password?id='+ usuario.id + '&t=' + newToken;
                ...
                var mailOptions = {
                  from: "myApplication <info@ekii.co>", // sender address
